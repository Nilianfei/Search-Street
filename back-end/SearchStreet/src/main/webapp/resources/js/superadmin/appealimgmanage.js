...
function searchAppealImgInfo() {
	var appealImgId = $("#searchInfoHd").val();
	if (appealImgId != '') {
		listAppealImgByAppealId();
	} else {
		listAppealImgs();
	}
}
...
function listAppealImgByAppealId() {
	var appealId = $("#searchInfoHd").val();
	if (appealId == "") {
		return;
	}
	var urlBuffer = new StringBuffer();
	urlBuffer.append("searchappealimgsbyappealid?appealId=");
	urlBuffer.append(appealId + "&");
	$('#urlHd').val(urlBuffer.toString());
	var obj = $("#appealImgManagementTable").datagrid("getPager").pagination(
			"options");
	getDataByPageRows(obj.pageNumber, obj.pageSize);
}

function listAppealImgs() {
	var urlBuffer = new StringBuffer();
	urlBuffer.append("listappealimgs?");
	$('#urlHd').val(urlBuffer.toString());
	var obj = $("#appealImgManagementTable").datagrid("getPager").pagination(
			"options");
	getDataByPageRows(obj.pageNumber, obj.pageSize);
...
function ajaxTable() {
	// 加载表格
	$('#appealImgManagementTable').datagrid(
			{
				toolbar : [ {
					text : '查询条件'
				} ],
				...
				onClickRow : function(rowIndex, rowData) {
					// 取消选择某行后高亮
					$('#appealImgManagementTable').datagrid('unselectRow',
							rowIndex);
				},
				onLoadSuccess : function() {
					var value = $('#appealImgManagementTable')
	...
	searchBoxBuffer
			.append('id="appealImgManagementSearch_searchBox" onchange="changeSearchInfo()"');
	searchBoxBuffer.append(' style="resize: none; width: 200px"></input>');
	searchBoxBuffer
			.append('<input type="button" id="searchBtn" value="按求助ID搜索"');
	searchBoxBuffer
			.append('style="margin-right: 0.5em;" onclick="searchInfo()"/>');
	var addButtonBuffer = new StringBuffer();
	addButtonBuffer
			.append('<input type="button" id="addAppealImg" value="添加求助图片"');
	addButtonBuffer
	...
	// 获取DataGrid分页组件对象
	var p = $("#appealImgManagementTable").datagrid("getPager");
	// 设置分页组件参数
	$(p).pagination({
...
function getDataByPageRows(pageNum, rowsLimit) {
	...
	$.ajax({
		...
		success : function(jsonObj, textStatus, jqXHR) { // 请求成功，将返回的数据（一页的记录数）绑定到
			// datagrid控件
			$('#appealImgManagementTable').datagrid('loadData', jsonObj);
		}
	});// ajax
}

function changeSearchInfo() {
	var info = $('#appealImgManagementSearch_searchBox').val();
	$('#searchInfoHd').val(info);
}

function searchInfo() {
	var info = $('#appealImgManagementSearch_searchBox').val().trim();
	$('#searchInfoHd').val(info);
	if (info == "") {
		searchAppealImgInfo();
	}
...
function optFormater(value, row, index) {
	var appealImgId = row.appealImgId;
	var add = '<a href="javascript:delAppealImg(' + appealImgId + ')">删除</a>';
	return add;
};
...
function openDialog_add() {
	appealImgManagementAddReset();
	$('#appealImgManagementAdd').dialog('open');
}
// 关闭对话框
function closeDialog_add() {
	$('#appealImgManagementAdd').dialog('close');
}
// 根据用户id查询用户的信息
function appealImgManagementAddReset() {
	$("#appealImgManagementAdd_message").html("");
	$('#form_appealImgManagementAdd').form('load', {
		appealImgId : null,
		appealId : null,
		imgAddr : null
	});
...
// 执行用户编辑操作
function appealImgManagementAdd() {
	$('#form_appealImgManagementAdd').form('submit', {
		url : 'addappealimg',
		onSubmit : function() {
		...
		success : function(data) {
			...
			if (data.success) {
				var messgage = "添加成功!";
				searchAppealImgInfo();
				$("#appealImgManagementAdd_message").html(messgage);
			} else {
 ...
 * @return
 */
function delAppealImg(appealImgId) {
	$.ajax({
		async : false,
		cache : false,
		type : 'post',
		dataType : "json",
		url : 'delappealimg?appealImgId=' + appealImgId,// 请求的action路径
